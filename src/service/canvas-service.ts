...
function pencil(canvas: canvasContext, properties: cursorProperties): CanvasRenderingContext2D | null {
...
function rect(canvas: canvasContext, properties: cursorProperties, isFill = false): CanvasRenderingContext2D | null {
  if (!canvas.ctx) return null;
  if (isFill) {
    canvas.ctx.fillRect(
...
function circle(canvas: canvasContext, properties: cursorProperties, isFill = false): CanvasRenderingContext2D | null {
  if (!canvas.ctx) return null;
  canvas.ctx.beginPath();
  ...
  canvas.ctx.arc(properties.cursorStartPos.x, properties.cursorStartPos.y, radius, 0, 2 * Math.PI);
  if (isFill) {
    canvas.ctx.fill();
  } else {
    canvas.ctx.stroke();
